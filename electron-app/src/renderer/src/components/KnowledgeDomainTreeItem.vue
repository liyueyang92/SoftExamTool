...
function toggleExpand() {
  if (hasChildren) emit('toggle-expand', props.node.id)
}
...
<template>
  <div class="tree-item">
    ...
    >
      ...
      <span class="chevron" :class="{ expanded: isExpanded, invisible: !hasChildren }" @click.stop="toggleExpand">
        ▸
      </span>

      <!-- Node name (clickable) -->
      <span class="node-name" @click="emit('select', node.id)">
        {{ node.name }}
      </span>

      <!-- Level badge -->
      <span class="badge level-badge" :class="getLevelClass(node.level)">
      ...
      <span class="node-actions">
        ...
        >✎</button>
        <button
          class="btn-xs btn-ghost btn-danger"
          title="删除"
...
.children {
  /* empty — tree indentation handled by paddingLeft on node-row */
}
</style>
