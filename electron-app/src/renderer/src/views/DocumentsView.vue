    ...
    <div class="main-layout">
      ...
      <div class="doc-list">
        ...
        <div v-else-if="store.documents.length === 0" class="empty-tip">
          ...
          <div>还没有导入文档</div>
          <div style="font-size:12px;margin-top:4px;color:#64748b">点击上方「导入 PDF」开始</div>
        </div>
...
.doc-list { background: #1e293b; border: 1px solid #334155; border-radius: 10px; overflow-y: auto; display: flex; flex-direction: column; gap: 0; }
.doc-item { display: flex; align-items: center; gap: 10px; padding: 12px; border-bottom: 1px solid #334155; cursor: pointer; transition: background 0.15s; }
.doc-item:hover { background: #1a2740; }
.doc-item.active { background: #1e3a5f; }
.doc-icon { font-size: 24px; flex-shrink: 0; }
.doc-info { flex: 1; min-width: 0; }
.doc-title { font-size: 13px; font-weight: 600; color: #e2e8f0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.doc-meta { font-size: 11px; color: #64748b; margin-top: 2px; }
.icon-btn { background: none; border: 1px solid #475569; border-radius: 4px; color: #94a3b8; width: 24px; height: 24px; cursor: pointer; display: flex; align-items: center; justify-content: center; font-size: 12px; flex-shrink: 0; }
.icon-btn.danger:hover { border-color: #f87171; color: #f87171; }

.chunks-panel { background: #1e293b; border: 1px solid #334155; border-radius: 10px; overflow: hidden; display: flex; flex-direction: column; }
.chunks-header { display: flex; align-items: center; justify-content: space-between; padding: 12px 16px; border-bottom: 1px solid #334155; }
.chunks-header h3 { font-size: 15px; font-weight: 600; color: #e2e8f0; }
.chunk-count { font-size: 12px; color: #64748b; }
.chunks-list { flex: 1; overflow-y: auto; padding: 8px; display: flex; flex-direction: column; gap: 8px; }
.chunk-item { background: #0f172a; border: 1px solid #334155; border-radius: 8px; padding: 12px; }
.chunk-page { font-size: 11px; color: #64748b; margin-bottom: 6px; }
.chunk-content { font-size: 13px; color: #cbd5e1; line-height: 1.6; max-height: 120px; overflow: hidden; position: relative; }
.chunk-content::after { content: ''; position: absolute; bottom: 0; left: 0; right: 0; height: 30px; background: linear-gradient(transparent, #0f172a); }
...
.empty-tip { text-align: center; padding: 48px; color: #475569; font-size: 13px; }
</style>
