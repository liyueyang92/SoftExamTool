...
function buildImportOptions() {
  ...
  if (Number.isNaN(top) || top < 0 || top >= 100) throw new Error('顶部裁剪比例需在 0~99 之间')
  if (Number.isNaN(bottom) || bottom < 0 || bottom >= 100) throw new Error('底部裁剪比例需在 0~99 之间')
      ...
      <div class="modal import-modal">
        ...
        <div class="modal-body">
          <div class="form-grid">
            ...
            <label class="field">
              <span>结束页</span>
              <input v-model="endPageInput" type="number" min="1" step="1" placeholder="留空=最后一页" />
            </label>
          </div>
...
.doc-list { background: var(--c-panel); border: 1px solid var(--c-border); border-radius: 10px; overflow-y: auto; display: flex; flex-direction: column; }
...
.modal-backdrop { position: fixed; inset: 0; background: rgba(0, 0, 0, 0.72); display: flex; align-items: center; justify-content: center; z-index: 100; }
.modal { background: var(--c-panel); border: 1px solid var(--c-border); border-radius: 12px; display: flex; flex-direction: column; }
.import-modal { width: min(920px, calc(100vw - 32px)); max-height: calc(100vh - 48px); overflow: hidden; }
...
.modal-body { flex: 1; min-height: 0; overflow-y: auto; padding: 18px 20px; display: flex; flex-direction: column; gap: 14px; }
...
.hint-row { display: flex; justify-content: space-between; gap: 12px; font-size: 12px; color: var(--c-text-3); }
.preview-toolbar { display: flex; align-items: flex-end; justify-content: space-between; gap: 12px; }
.preview-page-field { width: 180px; }

.preview-panel { border: 1px solid var(--c-border); border-radius: 10px; overflow: hidden; background: #101722; display: flex; flex-direction: column; min-height: 0; }
.preview-header { padding: 10px 14px; border-bottom: 1px solid var(--c-border); font-size: 13px; font-weight: 600; color: var(--c-text); }
.preview-content {
  margin: 0;
  padding: 14px;
  min-height: 240px;
